...
function isValidMonthKey(key) {
  return typeof key === "string" && /^\d{4}-\d{2}$/.test(key) && isValidDayKey(`${key}-01`);
}

function dateFromKey(key) {
  if (!isValidDayKey(key)) return new Date(Number.NaN);
  const [year, month, day] = key.split("-").map(Number);
...
function loadState() {
  try {
    ...
    if (isValidDayKey(saved.selectedDate)) fresh.selectedDate = saved.selectedDate;
...
function sanitizeExtensions(value) {
  ...
  clean.attachments = Array.isArray(value.attachments) ? value.attachments.map(item => {
    ...
    item && isValidDayKey(item.dateKey) && item.dataURL.length <= 2_200_000
  ).slice(-30).map(item => ({
  ...
  clean.lastEmailedMonth = isValidMonthKey(value.lastEmailedMonth) ? value.lastEmailedMonth : "";
  ...
  if (value.entryClients && typeof value.entryClients === "object") {
    for (const [date, clientID] of Object.entries(value.entryClients)) {
      if (isValidDayKey(date) && validClientIDs.has(clientID)) clean.entryClients[date] = clientID;
    }
  }
  ...
  if (value.dailyNotes && typeof value.dailyNotes === "object") {
    for (const [date, note] of Object.entries(value.dailyNotes)) {
      if (isValidDayKey(date) && typeof note === "string" && note.trim()) clean.dailyNotes[date] = note.trim().slice(0, 1000);
    }
  }
  clean.closedDayKeys = Array.isArray(value.closedDayKeys)
    ? [...new Set(value.closedDayKeys.filter(isValidDayKey))].slice(-1500)
    : [];
...
function sanitizeWorkSeconds(values) {
  ...
  for (const [date, seconds] of Object.entries(values)) {
    ...
    if (isValidDayKey(date) && Number.isFinite(value) && value > 0) clean[date] = Math.floor(value);
  }
  return clean;
}
...
function sanitizeActiveTimer(timer) {
  ...
  if (!timer || !isValidDayKey(timer.date) || !Number.isFinite(startedAt)) return null;
  if (startedAt <= 0 || startedAt > Date.now() + 60_000) return null;
  return { date: timer.date, startedAt };
}

function sanitizeTasks(tasks) {
  if (!Array.isArray(tasks)) return [];
  return tasks.filter(task => task && categories.some(category => category.id === task.categoryID) && isValidDayKey(task.dueDateKey))
    .map(task => ({
...
function sanitizeInvoices(invoices) {
  ...
  return invoices.filter(item => item && isValidMonthKey(item.monthKey)).map(item => ({
    id: String(item.id || crypto.randomUUID()),
...
function sanitizeEntries(entries) {
  ...
  for (const [date, counts] of Object.entries(entries)) {
    if (!isValidDayKey(date) || !counts || typeof counts !== "object") continue;
    const cleanCounts = {};
    for (const category of categories) {
...
function workedSecondsForMonth(monthDate, now = Date.now()) {
  const prefix = `${monthKey(monthDate)}-`;
  let seconds = Object.entries(state.workSeconds || {})
    .filter(([date]) => date.startsWith(prefix))
    .reduce((sum, [, value]) => sum + Math.max(0, Number(value) || 0), 0);
  if (state.activeTimer?.date.startsWith(prefix)) {
    seconds += Math.max(0, Math.floor((now - state.activeTimer.startedAt) / 1000));
  }
  return seconds;
}

function totalWorkedSeconds(now = Date.now()) {
  let seconds = Object.values(state.workSeconds || {})
    .reduce((sum, value) => sum + Math.max(0, Number(value) || 0), 0);
  if (state.activeTimer) seconds += Math.max(0, Math.floor((now - state.activeTimer.startedAt) / 1000));
  return seconds;
}
...
function stopActiveTimer() {
  ...
  const elapsed = Math.max(0, Math.floor((Date.now() - state.activeTimer.startedAt) / 1000));
  if (elapsed > 0) state.workSeconds[state.activeTimer.date] = (state.workSeconds[state.activeTimer.date] || 0) + elapsed;
...
function normalizedImportDate(value) {
  ...
  if (isValidDayKey(clean)) return clean;
  const match = clean.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})$/);
  if (!match) return null;
  const normalized = `${match[3]}-${match[2].padStart(2, "0")}-${match[1].padStart(2, "0")}`;
  return isValidDayKey(normalized) ? normalized : null;
}
...
function parseEuroCents(value) {
  ...
  if (lastComma >= 0 && lastDot >= 0) {
    ...
    clean = clean.split(thousandsSeparator).join("").replace(decimalSeparator, ".");
  } else if (lastComma >= 0) {
...
function importCSVText(text) {
  ...
  lines.forEach((line, index) => {
    ...
    const rateCents = parseEuroCents(cells[3]);
    ...
    if (state.entryRates[date][category.id] === undefined) state.entryRates[date][category.id] = rateCents ?? rateFor(category.id);
...
function applyAccountSession(session, fallbackProvider = "account") {
  const accessToken = normalizedAccountToken(session?.access_token);
  if (!accessToken) throw new Error("invalid-session");
  account.accessToken = accessToken;
  account.refreshToken = normalizedAccountToken(session?.refresh_token) || account.refreshToken;
...
async function handleAccountCallback() {
  ...
  const accessToken = normalizedAccountToken(parameters.get("access_token"));
  if (!accessToken) {
    clearAccountSession();
    account.status = "Die Anmeldung lieferte keine gültige Sitzung. Bitte erneut versuchen.";
    saveAccount();
    renderAccountSettings();
    return true;
  }
  try {
    ...
    clearAccountSession();
    account.status = "Die Anmeldung konnte nicht bestätigt werden. Bitte erneut anmelden.";
    saveAccount();
    renderAccountSettings();
  }
  return true;
...
async function handleAccountStartup() {
  const startupToken = account.accessToken;
  try {
    if (await handleAccountCallback()) return;
    if (!startupToken || account.accessToken !== startupToken || !validAccountConfiguration()) return;
    await loadAccountUser();
    renderAccountSettings();
  } catch {
    if (account.accessToken !== startupToken) return;
    account.accessToken = "";
    account.refreshToken = "";
    account.expiresAt = 0;
    account.user = null;
